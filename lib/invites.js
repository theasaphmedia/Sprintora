...
// Update an invite record's status to reflect current membership state
// ("left" on self-removal, "removed" on owner-initiated removal), so the
// owner's "Invites sent" list reflects reality instead of freezing at
// "accepted" forever. Best-effort: older members added before this existed,
// or before an invite record was created for them, won't have a matching
// doc — that's fine, this just no-ops rather than failing the caller's
// actual membership-removal write (which already succeeded by this point).
export async function markInviteStatus(projectId, email, status) {
  try {
    await updateDoc(doc(db, "invites", inviteId(projectId, email.toLowerCase())), { status });
  } catch (err) {
    console.error("markInviteStatus: couldn't update invite record", err);
  }
}
...
export async function acceptPendingInvites(user) {
  ...
  try {
    ...
    // Most common cause: the security rules require email_verified == true
    // to read invites matched by email, and the cached ID token doesn't
    // carry that claim yet (it's not auto-refreshed just because
    // user.emailVerified flipped to true) — logged so this is actually
    // debuggable instead of a silent, invisible no-op.
    console.error(
      "acceptPendingInvites: getDocs denied (often stale ID token — try 'I've verified — refresh')",
      e
    );
    return;
  }
...
export async function inviteTeammate({ projectId, projectName, email, invitedBy }) {
  ...
  if (!usersSnap.empty) {
    ...
    // Record this too (previously only the "no account yet" branch created
    // an invite doc), so every member — not just ones who signed up via a
    // pending invite — shows up in the owner's "Invites sent" list with an
    // accurate, trackable status.
